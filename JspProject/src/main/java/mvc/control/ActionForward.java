 ...
 * 
 * InsertAction, ListAction, InfoAction의 비즈니스 로직 수행 후 
 * controlServlet에게 반환하는 객체를 의미함
 ...
 *  1. 사용자의 요청을 받는다.
 *  2. 사용자의 요청을 분석한다.
 *  3. 사용자의 요청을 처리할 자바빈을 생성하고 비즈니스 로직이 구현된 메소드를 실행한다.
 *  4. 비즈니스 로직 수행 후 사용자의 요청을 JSP페이지나 혹은 특정 URL로 이동시킴
 *  
 * 요청 파라미터를 명령어로 전달하는 방식으로 구현
 * 위의 방법으로 개발시 필요한 클래스
 *  1. controlServlet에게 반환하는 객체를 의미함(controller의 역할 수행하는 서블릿)
